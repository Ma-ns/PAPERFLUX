...
const Sidebar: React.FC = () => {
  ...
  const [ selected, isSelected] = useState(pathname);
      ...
      <div className='h-[62%] w-full'>
        ...
        <div className='flex flex-col items-start justify-start gap-y-2'>
          {PAGES_DATA.map((page, pageIndex) => (
            ...
            (selected === page.href ? 'bg-purple-100 px-4 py-3 rounded-lg' :
              'px-2 py-2 hover:bg-slate-200/80 rounded-lg'
            )}>
              <div>
                { page.title === 'Documentos' &&
                  <svg xmlns="http://www.w3.org/2000/svg" width="25" height="34" viewBox="0 0 25 34" fill="none" className={'transition-all ' + (selected === page.href ? 'fill-purple-800 size-8' : 'fill-slate-700 group-hover/button:fill-purple-700 size-7')}>
                ...
                { page.title === 'Colaboradores' &&
                  <svg xmlns="http://www.w3.org/2000/svg" width="37" height="37" viewBox="0 0 37 37" fill="" className={'transition-all ' + (selected === page.href ? 'fill-purple-800 size-8' : 'fill-slate-700 group-hover/button:fill-purple-700 size-7')}>
                ...
                { page.title === 'Relatórios' &&
                  <svg xmlns="http://www.w3.org/2000/svg" width="37" height="37" viewBox="0 0 37 37" fill="none" className={'transition-all ' + (selected === page.href ? 'fill-purple-800 size-8' : 'fill-slate-700 group-hover/button:fill-purple-700 size-7')}>
              ...
              <div className={'font-medium tracking-wide text-slate-700 transition-all ' + 
                (selected === page.href ? 'text-lg ' : 'text-base')}>
                {page.title}
              </div>
            </button>
          ))}
